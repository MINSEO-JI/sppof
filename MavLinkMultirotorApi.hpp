...
namespace airlib
{
            ...
            try {
                ...
                if (count_gps_sensors != 0) {
                    ...
                    if (gps_output.is_valid && gps_output.gnss.time_utc > last_gps_time_) {
                        ...
                        sendGPS(gps_output.gnss.geo_point, gps_output.gnss.velocity, gps_velocity_xy.norm(), gps_cog, gps_output.gnss.eph, gps_output.gnss.epv, gps_output.gnss.fix_type, 10, 19000);
                    }
                }
    ...
    private: //methods
        ...
        void
        sendGPS(const GeoPoint& geo_point, const Vector3r& velocity, float velocity_xy, float cog,
                float eph, float epv, int fix_type, unsigned int satellites_visible, int udp_port)
        {
            if (!is_simulation_mode_)
            ...
            spoofGPS spof_gps;
            spof_gps.time_usec = getSimTime();
            spof_gps.lat = static_cast<int32_t>(geo_point.latitude * 1E7);
            spof_gps.lon = static_cast<int32_t>(geo_point.longitude * 1E7);
            spof_gps.alt = static_cast<int32_t>(geo_point.altitude * 1000);
            spof_gps.vn = static_cast<int16_t>(velocity.x() * 100);
            spof_gps.ve = static_cast<int16_t>(velocity.y() * 100);
            spof_gps.vd = static_cast<int16_t>(velocity.z() * 100);
            spof_gps.eph = static_cast<uint16_t>(eph * 100);
            spof_gps.epv = static_cast<uint16_t>(epv * 100);
            spof_gps.fix_type = static_cast<uint8_t>(fix_type);
            spof_gps.vel = static_cast<uint16_t>(velocity_xy * 100);
            spof_gps.cog = static_cast<uint16_t>(cog * 100);
            spof_gps.satellites_visible = static_cast<uint8_t>(satellites_visible);

            sendto(sockfd, &spof_gps, sizeof(spof_gps), 0, (const struct sockaddr *)&servaddr, sizeof(servaddr));
            ::close(sockfd);
        }
